...
function createGraph(selectorActivities, logs_filtered_by_period_and_activity, event_mapping, users) {
    ...
    min.forEach((user) => {
        ...
        all_min_users_ways.push(userData) 
    })

    let all_max_users_ways = []
    max.forEach((user) => {
        let userData = []
        let auxEventList = []
        logs_to_be_worked.forEach((current_data) => {
        ...
        all_max_users_ways.push(userData)
    })


    generateGraph(domainsContent, all_min_users_ways, all_max_users_ways)
}
